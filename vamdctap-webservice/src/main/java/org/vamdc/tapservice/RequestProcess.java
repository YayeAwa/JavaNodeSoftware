...
public class RequestProcess implements RequestInterface {
	...
	public RequestProcess (String query, Collection<Restrictable> restrictables){
		initRequest(
				...
				new QueryImpl(query,restrictables));
	}
	...
	private static ResponseBuilder setHeaders(ResponseBuilder base,XSAMSMetrics metrics){
		...
		base.header("Content-Disposition", "attachment; filename="+Settings.getValue(Settings.SETTING_XSAMSID_PREFIX)+(new Date().toString().replace(" ","_"))+".xsams");
		return base;
	}
	
	/**
	 * Set headers, based on estimation.
	 * Is used for HEAD request processing and will be used for stream generation.
	 * @param base response
	 * @param metrics estimated response metrics map
	 * @return response with appended headers
	 */
	private ResponseBuilder setEstimateHeaders(ResponseBuilder base, Map<HeaderMetrics, Integer> metrics){
		for (HeaderMetrics hdr: HeaderMetrics.values()){
			String header = hdr.name().replace("_", "-");//Headers contain "-", we use "_" for variable names
			String value = "0";
