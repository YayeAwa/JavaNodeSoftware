...
public class PrefixHandler {
	 ...
	 * @param restrictsList
	 */
	public PrefixHandler(Collection<RestrictExpression> restrictsList){
		for (RestrictExpression rexp:restrictsList){
			if (rexp instanceof RestrictExpressionImpl)
				assignPrefixPass1((RestrictExpressionImpl) rexp);
		}
		if (reactIndex.size()>1 || prodIndex.size()>1){//If we have more than one reactant or product, we need a second pass to assign indexes.
			...
			for (RestrictExpression rexp:restrictsList){
				if (rexp instanceof RestrictExpressionImpl)
					assignPrefixPass2((RestrictExpressionImpl) rexp);
			}
		}
	 ...
	 * @param rexp
	 */
	private void assignPrefixPass1(RestrictExpressionImpl rexp){
	 ...
	 * @param rexp
	 */
	private void assignPrefixPass2(RestrictExpressionImpl rexp){
